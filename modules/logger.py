...
def init_logging(color: bool=True, oar_id = None, cache_path: str='_out'):
    ...
    if oar_id is not None:
        ...
        file_handler.setFormatter(logging.Formatter(f'%(asctime)s |%(levelname)8s| (%(filename)s:%(funcName)s:%(lineno)d) %(message)s'))
