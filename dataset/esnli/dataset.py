...
from torch.utils.data import Dataset, MapDataPipe
...
def ESNLIDataPipe(root:str, split:str, n_data=-1):
	"""
	eSNLI dataset. Notice: in testset, we return only the Highlight text for person 1.
	Args:
		root (str): Directory where the datasets are saved. Default: os.path.expanduser('~/.torchtext/cache')
		split ():split or splits to be returned. Can be a string or tuple of strings. Default: (`train`, `valid`, `test`)

	Returns:

	"""
	csv_path = download_format_dataset(root, split, n_data)
	dp = IterableWrapper([csv_path])
	dp = FileOpener(dp, mode='b')
	return dp.parse_csv_as_dict(delimiter=',')


def download_format_dataset(root:str, split:str, n_data:int=-1):
	"""
	...
	subset = '' if n_data < 0 else 'subset_'+ str(n_data)
	zip_path = path.join(root, ZIP_FILEPATH)
	csv_path = path.join(root, _EXTRACTED_FILES[split])
	subset_csv_path = path.join(root, subset, _EXTRACTED_FILES[split])
	...
	if n_data > 0 and not path.exists(subset_csv_path):
		os.makedirs(path.join(root, subset), exist_ok=True)
		if df is None: df = pd.read_csv(csv_path)
		df = df[:n_data]
		df.to_csv(subset_csv_path, index=False, encoding='utf-8')
...
class ESNLIDataset(MapDataPipe):
...
if __name__=='__main__':
	...
	trainset, valset = ESNLIDataPipe(root=cache_path, split=('train', 'valid'))
