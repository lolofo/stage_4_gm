...
class BertNliRegu(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        # score for the metrics -- all detach
        padding = torch.tensor([0]).to(self.device).detach()
        ...
        a_true = torch.flatten(train_batch["annotations"])[non_pad_pos].clone().detach()

        log.debug(f"{reg_term['pen'].requires_grad}")
    ...
    def training_step_end(self, output):
        ...
        self.log("train_/reg", output["reg_term"], on_step=True, on_epoch=False, logger=True, prog_bar=True)
